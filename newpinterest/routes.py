...
def perfil(id_usuario):
    if int(id_usuario) == int(current_user.id): #usuário dentro do seu perfil
        form_foto = FormFoto()
        if form_foto.validate_on_submit():
            arquivo = form_foto.foto.data
            '''Às vezes o aquivo pode vir com caractere especial e isso pode trazer erro ao código
            Utilizar secure_filename é interessante'''
            nome_seguro = secure_filename(arquivo.filename)
            #salvar aquivo na pasta fotos_post 
...
@login_required
def feed():
    fotos = Foto.query.order_by(Foto.data_criacao.desc()).all() #.desc() = decrescente
